...
export default function DataAvailabilityMatrix({ nightMode = false }: DataAvailabilityMatrixProps) {
  ...
  return (
    <div className={`${nightMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} rounded-2xl shadow-fancy border p-6 hover-lift transition-colors duration-300`}>
      {/* Instructions */}
      <div className={`mb-4 p-4 ${nightMode ? 'bg-primary-900/30 border-primary-600' : 'bg-primary-50 border-primary-400'} border-l-4 rounded-r-lg`}>
        <div className="flex items-start">
          <svg className={`w-5 h-5 ${nightMode ? 'text-primary-400' : 'text-primary-600'} mt-0.5 mr-2 flex-shrink-0`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            <p className={`text-sm ${nightMode ? 'text-primary-200' : 'text-primary-900'}`}>
              ...
              Tags like <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${nightMode ? 'bg-primary-800/50 text-primary-200' : 'bg-primary-100 text-primary-800'} mx-1`}>mat</span> or 
              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${nightMode ? 'bg-primary-800/50 text-primary-200' : 'bg-primary-100 text-primary-800'} mx-1`}>pat</span> indicate maternal/paternal haplotypes. 
